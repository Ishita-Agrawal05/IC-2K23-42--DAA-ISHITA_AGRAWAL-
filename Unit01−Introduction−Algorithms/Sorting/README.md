...
compares insertion sort, recursive insertion sort, merge sort, quick sort, and
heap sort using the same random inputs. It verifies sorted output, measures
average execution time for different input sizes, saves the comparison results
as CSV, and generates an `n` versus time plot.
...
Recursively sort the first `n - 1` elements, then insert the final element into
the sorted prefix. The implementation uses one recursive call per element, so
its auxiliary call-stack space is `O(n)`.
...
| Recursive Insertion Sort | O(n) | O(n^2) | O(n^2) | O(n) |
...
same input to every algorithm, repeats each timing, and reports the mean
execution time. The recursive implementation is benchmarked through `n = 800`
because its recursion depth is linear and Python's default recursion limit is
approximately 1000. The graph and CSV are generated by running:
